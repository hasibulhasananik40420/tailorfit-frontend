...
import { useDeleteIndividualOrderMutation } from "../../../redux/api/individualOrderApi";
...
const ActionsButtonForAllOrders = ({ isOpen, setIsOpen, id, industry }: UserModalProps) => {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const result = await Swal.fire({
        ...
        imageUrl: 'https://res.cloudinary.com/diam8ogqq/image/upload/v1724340374/warning_ckqqz8.png',  
        imageWidth: 48,                            
        imageHeight: 48,                         
    ...
    <div>
      {isOpen && (
        <div
          ...
          className=" h-[104px] bg-white w-[126px] rounded-[5px] p-[10px]"
        >
          <div>
            <button className="bg-white w-[106px] text-switchColor text-[16px] font-Poppins font-medium leading-5 flex items-center gap-1 h-10 pl-2 rounded hover:bg-activeDhcolor duration-300">
              <svg
...
export default ActionsButtonForAllOrders;
