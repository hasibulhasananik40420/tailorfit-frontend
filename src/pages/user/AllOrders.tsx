...
const AllOrders = () => {
  ...
  const handleStatusOfCompany = async (orderId: string, status: Status) => {
    const info = {
      _id: orderId,
      orderStatus: status.text,
      orderBGColor: status.color,
  ...
  const filters = useAppSelector((state: RootState) => state.allfilter);
  ...
  const filteredOrdersByStatus = newArray?.filter((order: any) => {
  ...
  return (
    <div className="relative bg-white rounded-[10px] border-[1px] border-borderColor 2xl:p-[30px] lg:p-[15px] md:p-5 p-4">
      <>
        {/* {searchQuery && (
        <div
      ...
      {filteredOrdersByStatus?.length > 0 ? (
        <div>
          <div className="flex items-center gap-3 2xl:pb-[30px] lg:pb-[15px] md:pb-5 pb-4">
            <Link to={`/${currentData?.role}/dashboard`}>
              <GoArrowLeft className="size-6 text-black" />
            </Link>
            <h1 className="2xl:text-[24px] lg:text-[18px]  text-[14px]  text-secondaryColor font-Poppins font-semibold">
              ( {orderName})
            </h1>
          </div>
              ...
              <tbody className="">
                {filteredOrdersByStatus?.map(
                  (order: any, orderIndex: number) => (
                    <>
                      ...
                      >
                        ...
                        >
                          {order?.industry ? (
                            <Link className="hover:text-primaryColor duration-100"
                        ...
                        <th
                          scope="row"
                          className=" 2xl:px-0 2xl:py-3  py-2 font-Poppins font-medium text-[#353535] whitespace-nowrap 2xl:text-[18px] 2mid75:text-[16px] 2large:text-[16px] lg:text-[14px]  2makbook:text-[10px] text-[14px] 2xl:max-w-[230px] lg:max-w-[180px] 2mid75:max-w-[170px] 2large:max-w-[150px] max-w-[130px] overflow-hidden text-ellipsis "
                        >
                          {order?.industry ? (
                            <Link
                              to={`/${currentData?.role}/order-detail/${order._id}`}
                            ...
                            >
                              <div className="flex items-center gap-1 hover:text-primaryColor duration-100">
                                <GrGroup className="size-4 text-primaryColor" />
                            ...
                            >
                              <div className="flex items-center gap-1 hover:text-primaryColor duration-100">
                                <>
                                  <FiUser className="size-4 text-primaryColor" />
                                </>
                                {order?.customerName}
                              </div>
                            </Link>
                        ...
                        >
                          ...
                          />
                          {/* {order?.industry ? 'comapny' : 'indevisual'} */}
                        </th>
                        <th
                          scope="row"
                          className="2xl:px-6 2xl:py-3 px-3 py-2 font-Poppins font-normal text-secondaryColor whitespace-nowrap 2xl:text-[18px] 2large:text-[16px] lg:text-[14px] 2mid75:text-[16px] 2makbook:text-[10px] text-[14px]"
                        ...
                        >
                           {/* ${ 
                            dropdownOpen === order?._id && " pointer-events-none cursor-pointer"  
                          } */}
                                ...
                                {status.map((status, id: number) => {
                                  return (
                                    <div
                                      ...
                                      onClick={() => {
                                        if (order?._id && order?.industry) {
                                          handleStatusOfCompany(
                                            order._id,
                                            status
                                          );
                                        } else {
                          ...
                          <div className="flex justify-between items-center 2xl:gap-[15px] gap-[10px]">
                            ...
                            <div className="w-[37px] h-[37px] rounded-[6px] bg-[#F6F6F6] flex justify-center items-center cursor-pointer">
                              <HiDotsVertical
                                ...
                                onClick={() =>
                                  toggleActionModal(order?._id, orderIndex)
                                }
                                className="size-5 text-[#333333]"
                              />

                              {actionModalOpen.id === order?._id && (
                                <div
                                ...
                                >
                                  <ActionButtonModal
                                    isOpen={actionModalOpen.id === order?._id}
                                    setIsOpen={() =>
                                      setactionModalOpen({
                                        id: null,
                                        index: null,
                  ...
                  {actionModalOpen.id === order?.orderId && (
                    ...
                    >
                      <ActionsButtonForAllOrders
                        isOpen={actionModalOpen.id === order?.orderId}
                        setIsOpen={() =>
                          setactionModalOpen({ id: null, index: null })
                        }
                        id={order?._id}
        ...
        <div className="flex items-center justify-center gap-4 text-[24px] text-center text-switchColor font-Noto-Sans-Bengali font-medium  min-h-[600px]">
          ...
          <p>কোনো ফিল্টার রেজাল্ট পাওয়া যায়নি</p>
        </div>
      )}
    </div>
  );
